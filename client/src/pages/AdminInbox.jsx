...
import { Search, Inbox, Eye, Mail, MessagesSquare, ArrowLeft } from 'lucide-react';

/**
 * Inbox — questions students sent through the "Talk to ICT" page.
 * Two-pane messenger: conversations on the left, the live thread on the
 * right. Messages, typing and "student opened the chat" appear instantly
 * (websocket, with polling as automatic fallback). Closing a conversation
 * deletes its entire history — only then, for both sides.
 */
export default function AdminInbox() {
  ...
  function load(q = query, filter = statusFilter) {
    ...
    api(`/staff/inbox${qs ? `?${qs}` : ''}`).then((d) => { setList(d.messages); setUnread(d.unread); }).catch((e) => setError(e.message));
  }
  ...
  const openMsg = list?.find((m) => m.id === openId) || null;
  const waitingCount = (list || []).filter((m) => !m.closed_at && (m.status === 'new' || m.last_sender === 'student')).length;
  const closedCount = (list || []).filter((m) => m.closed_at).length;

  return (
    <>
      <div className="pg-head">
        <div>
          <span className="pg-head__eyebrow">Live conversations</span>
          <h2 className="pg-head__title">Student questions</h2>
          <p className="pg-head__sub">
            {list
              ? <><strong>{waitingCount}</strong> waiting for a reply · <strong>{closedCount}</strong> closed · <strong>{unread}</strong> unread</>
              : 'Loading…'}
          </p>
        </div>
        <div className="pg-head__actions">
          <div className="inbox-search" style={{ background: '#fff' }}>
            <span aria-hidden="true"><Search size={15} /></span>
            <input
              value={query}
              onChange={(e) => { setQuery(e.target.value); setVisibleCount(30); }}
              placeholder="Search name, matric no, subject…"
          ...
          <select className="seg-select" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setVisibleCount(30); }} aria-label="Filter conversations">
      ...
      <div className={`inbox-split ${openId ? 'has-open' : ''}`}>
        {/* ------------------------- left: conversation list ------------------------- */}
        <div className="inbox-pane inbox-pane--list">
          <div className="inbox-pane__head" style={{ padding: '12px 14px 8px' }}>
            <strong>Conversations</strong>
            <div style={{ display: 'flex', gap: 8 }}>
              <select className="seg-select" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setVisibleCount(30); }} aria-label="Filter conversations" style={{ flex: 1 }}>
          ...
          <div className="inbox-pane__body">
            ...
            {shown.map((m) => {
              ...
              return (
                <button key={m.id} type="button"
                  className={`conv ${isActive ? 'is-active' : ''} ${waiting && m.last_sender === 'student' ? 'is-unread' : ''}`}
                  onClick={() => setOpenId(isActive ? null : m.id)}>
                  <span className="conv__avatar" aria-hidden="true">{m.sender_name?.split(/\s+/).map((w) => w[0]).slice(0, 2).join('').toUpperCase()}</span>
                  <span className="conv__body">
                    <span className="conv__top">
                      <span className="conv__name">{m.sender_name}</span>
                      <span className="conv__time">{fmtDateTime(m.last_msg_at || m.created_at)}</span>
                    </span>
                    <span className="conv__subject">{m.subject}</span>
                    <span className="conv__preview">
                      {m.closed_at
                        ? '— conversation closed —'
                        : m.last_msg_at
                          ? `${m.last_sender === 'staff' ? 'You: ' : ''}${(m.last_preview || m.message).slice(0, 60)}`
                          : m.message.slice(0, 60)}
                    </span>
                  </span>
                  <span className="conv__flag">
                    ...
                    {m.student_opened_at && !m.closed_at && (
                      <small title="The student has this conversation open right now" style={{ display: 'inline-flex', alignItems: 'center', gap: 3, color: 'var(--green)', fontWeight: 700, fontSize: '.7rem' }}>
                        <Eye size={11} />live
                      </small>
                    )}
                  </span>
            ...
            {list && list.length > visibleCount && (
              <div style={{ textAlign: 'center', padding: 12 }}>
                <button type="button" className="btn btn--outline btn--sm" onClick={() => setVisibleCount((n) => n + 30)}>
                  Show 30 more ({list.length - visibleCount} remaining)
                </button>
              </div>
            )}
          </div>
        ...
        {/* ------------------------- right: the thread ------------------------- */}
        <div className="inbox-pane inbox-pane--thread">
          <button type="button" className="inbox-back" onClick={() => setOpenId(null)}>
            <ArrowLeft size={15} /> All conversations
          </button>
          {!openMsg && (
            <div className="inbox-empty">
              <div>
                <div className="inbox-empty__art"><MessagesSquare size={36} /></div>
                <strong>Select a conversation</strong>
                <p>Choose a student on the left to read their question and reply live. You'll see when they're typing and when they have the chat open.</p>
              </div>
            </div>
          )}
          {openMsg && (openMsg.closed_at ? (
            <div className="inbox-thread-inpane">
              <div className="inbox-thread">
                <div className="inbox-thread__info">
                  <strong>{openMsg.subject}</strong>
                  <span>{openMsg.sender_name} · {openMsg.email}{openMsg.matric_no ? ` · ${openMsg.matric_no}` : ''}</span>
                </div>
            ...
            <LiveThread key={openMsg.id} id={openMsg.id} fallbackMeta={openMsg} onClosed={() => { setOpenId(null); load(); }} />
          ))}
        </div>
      </div>
    </>
...
function LiveThread({ id, fallbackMeta, onClosed }) {
  const { thread, live, send, sendTyping, close } = useLiveChat({ mode: 'staff', messageId: id });
